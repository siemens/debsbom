...
from spdx_tools.spdx.model.package import Package, ExternalPackageRefCategory
...
VALID_PACKAGE_MANAGER_REFS = ["maven-central", "npm", "nuget", "bower", "purl"]


class SpdxDeltaGenerator(DeltaGenerator):
    ...
    def _normalize_external_refs(self, package: Package):
        if not package.external_references:
            return
        valid_refs = []
        for ref in package.external_references:
            if ref.category == ExternalPackageRefCategory.PACKAGE_MANAGER:
                if ref.reference_type not in VALID_PACKAGE_MANAGER_REFS:
                    logger.warning(
                        f"Package '{package.name}': removing PACKAGE_MANAGER ref "
                        f"'{ref.reference_type}' (invalid type)"
                    )
                    continue
            valid_refs.append(ref)
        package.external_references = valid_refs

    def delta(self, base_sbom, target_sbom) -> Document:
        packages = {}
        non_purl_packages = []
        relationships = {}
        id_map = {}
        ...
        for package in target_sbom.packages:
            self._normalize_external_refs(package)
        ...
        for rel in target_sbom.relationships:
            ...
            if rel.spdx_element_id not in id_map or rel.related_spdx_element_id not in id_map:
                continue
        ...
        unique_packages = {p.spdx_id: p for p in packages}
