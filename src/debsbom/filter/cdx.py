...
class CdxSbomFilter(SbomFilter):
    ...
    def exclude(cls, document: Bom, *, binary_patterns=(), source_packages=()) -> dict:
        ...
        matching source package is removed together with the binaries built from
        it. Afterwards, packages that are no longer referenced by any remaining
        package are removed as well, until no more packages become unreferenced.
        """
        ...
        # binaries built from an excluded source, using the same binary -> source
        # mapping as the CycloneDX package resolver
        if matched_sources:
            resolver = CdxPackageResolver(document)
            for ref, package in resolver.packages_by_ref.items():
                if package.is_source() or not package.source:
                    continue
                source = (package.source.name, package.source.version[1])
