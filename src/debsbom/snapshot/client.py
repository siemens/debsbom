...
class SnapshotRemoteFile(RemoteFile):
    ...
    def _sanitize_path(path: str) -> str:
        """
        Ensure the path is absolute and does not traverse above root, then return it.
        Raises ValueError otherwise.
        """
        # normpath is purely lexical, so no filesystem is touched here
        normalized = posixpath.normpath(path.lstrip("/"))
        if not path.startswith("/") or normalized.split("/")[0] == "..":
