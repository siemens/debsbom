        ...
        for pkg in packages:
            if pkg["SPDXID"].endswith("binutils-2.40-2-srcpkg"):
                ...
                for ref in externalRefs:
                    ...
                    elif ref["referenceType"] == "vcsBrowser":
                        assert (
                            ref["referenceLocator"]
                            == "https://salsa.debian.org/toolchain-team/binutils/tree/binutils-2.40"
                        )
                ...
                for ref in externalRefs:
                    if ref["type"] == "vcs":
                        assert ref["url"].startswith(
                            "https://salsa.debian.org/toolchain-team/binutils"
                        )
