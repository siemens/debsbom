...
def test_fromfileinfo_sanitization():
    ...
    # parent components that remain below the root are allowed
    fileinfo_with_parent = dict(fileinfo, path="/pool/main/../p/pytest")
    assert (
        SnapshotRemoteFile.fromfileinfo(sdl, hash, fileinfo_with_parent).path
        == "/pool/main/../p/pytest"
    )
    ...
    for field, value in [
        ...
        ("path", "/pool/../../etc"),  # traverses above root
    ]:
        bad = dict(fileinfo)
        bad[field] = value
        with pytest.raises(ValueError):
